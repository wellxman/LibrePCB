...
protected:
  ...
  static WorkerResult
      threadWorker(FilePath fp, int options, int duration) noexcept {
  ...
  QFuture<WorkerResult>
      startWorkerThread(QThreadPool* pool, int options, int duration) noexcept {
#if (QT_VERSION >= QT_VERSION_CHECK(5, 4, 0))  // QtConcurrent::run(QThreadPool*,
                                               // ...) requires Qt>=5.4
    auto future = QtConcurrent::run(
        pool, threadWorker, mTempDbFilePath, options, duration);
#else
