...
std::unique_ptr<library::Package> PackageConverter::generate() const {
  std::shared_ptr<library::Footprint> footprint(new library::Footprint(
      mDb.getFootprintUuid(mPackage.getName()),
      ElementName("default"),
      ""));  // can throw

  std::unique_ptr<library::Package> package(new library::Package(
      mDb.getPackageUuid(mPackage.getName()),
      Version::fromString("0.1"),
      "LibrePCB",
      ElementName(mPackage.getName()),
      createDescription(),
      ""));  // can throw
  ...
  foreach (const parseagle::Circle& circle, mPackage.getCircles()) {
    ...
    bool isGrabArea = true;
    footprint->getCircles().append(std::make_shared<Circle>(
        Uuid::createRandom(),
        layerName,
        lineWidth,
        fill,
  ...
  foreach (const parseagle::SmtPad& pad, mPackage.getSmtPads()) {
    ...
    if (layerName == GraphicsLayer::sTopCopper) {
      ...
      throw Exception(
          __FILE__, __LINE__, QString("Invalid pad layer: %1").arg(*layerName));
    }
    Point pos = Point::fromMm(pad.getPosition().x, pad.getPosition().y);
    Angle rot = Angle::fromDeg(pad.getRotation().getAngle());
    ...
    std::shared_ptr<library::FootprintPad> fptPad(new library::FootprintPad(
        ...
        pos,
        rot,
        library::FootprintPad::Shape::RECT,
        width,
        height,
        UnsignedLength(0),
        side));
